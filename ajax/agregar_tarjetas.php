...
if (!empty($id) and !empty($cantidad)  and !empty($precio_venta) and ($cantidad>0) and ($precio_venta>0) )
{
		...
		$ESTADO = $row['descripcion'];
                 ...
                 //$id=$porciones[0];
    if($ESTADO==0){
        $insert_tmp=mysqli_query($con, "INSERT INTO tmp (id_producto,cantidad_tmp,precio_tmp,session_id,tienda,cod,Renglon_Presupuestario) VALUES ('$id','$cantidad',' $precio_venta','$session_id','$stock','$lote','ALMACENADO')");
    }else{            
        print"El bien ya ha sido asignado anteriormente.";
    }    
