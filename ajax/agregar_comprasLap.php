...
if (!empty($id) and !empty($cantidad) and !empty($precio_venta) and ($cantidad>0) and ($precio_venta>0) )
{
   ...
   // 1.1) if($numrows==0){
        $insert_tmp=mysqli_query($con, "INSERT INTO tmp1 (id_producto,cantidad_tmp,precio_tmp,session_id,tienda,cod,Renglon_Presupuestario,serie) VALUES ('$id','$cantidad','$precio_venta','$session_id','$stock','$cod','$cod2','$lote')");
    // 1.1) }else{            
...
$delete=mysqli_query($con, "DELETE FROM tmp1 WHERE id_tmp='".$id_tmp."'");
}
?>
...
<?php
	$sumador_total=0;
	$sql=mysqli_query($con, "select * from tmp1 where tmp1.session_id='".$session_id."' ORDER BY  `tmp1`.`id_tmp` ASC ");
	...
	$sumador_total+=$precio_total_r;//Sumador
		...
		<tr style="background:white;">
                        ...
                        <td class='text-right' style="background:white;"><textarea  id="cod2-<?php echo $id_tmp;?>" name="cod2-<?php echo $id_tmp;?>"  style="width:80px;background:#E6E6E6;text-align:right;height:40px;"><?php echo $lote;?></textarea></td>
                        ...
                        <td class='text-right' style="background:white;"><textarea  id="precio_tmp-<?php echo $id_tmp;?>" name="precio_tmp-<?php echo $id_tmp;?>" class="monto input" style="width:70px;background:#E6E6E6;text-align:right;height:40px;"><?php echo str_replace(",","",number_format($precio_venta_f,2));?></textarea></td>
