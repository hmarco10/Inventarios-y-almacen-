...
if (isset($_POST['estado'])){$estadoProducto=$_POST['estado'];}
if (isset($_POST['prof'])){$profesional=$_POST['prof'];}
...
{
        $count_query   = mysqli_query($con, "SELECT * FROM products where id_producto='$id'");
		$row= mysqli_fetch_array($count_query);
		$ESTADO = $row['descripcion'];
                 //$porciones = explode("----", $id);
                 //$id=$porciones[0];
    if($ESTADO==1){
        $insert_tmp=mysqli_query($con, "INSERT INTO tmp_descarga_tarjeta (id_producto,cantidad_tmp,precio_tmp,session_id,tienda,cod,Renglon_Presupuestario,profesional) VALUES ('$id','$cantidad',' $precio_venta','$session_id','$stock','$lote','ASIGNADO','$profesional')");
    }else{            
        print"El bien ya se encuentra almacenado.";
    }    
...
$delete=mysqli_query($con, "DELETE FROM tmp_descarga_tarjeta WHERE tmp_descarga_tarjeta.id_tmp='".$id_tmp."'");
}
?>
...
<tr style="background:orange;color:black;">
	...
	<th style="width:40%;" class='text-center'>BIEN ASIGANDO</th>
    <th style="width:10%;" class='text-center'>Profesional</th>
...
<?php
	$sumador_total=0;
	$sql=mysqli_query($con, "select * from tmp_descarga_tarjeta where tmp_descarga_tarjeta.session_id='".$session_id."' ORDER BY  `tmp_descarga_tarjeta`.`id_tmp` ASC ");
	...
	$sumador_total+=$precio_total_r;//Sumador
		...
		<tr style="background:white;">
            ...
            <td class='text-center'  style="background:white;"><textarea id="cantidad_tmp-<?php echo $id_tmp;?>" name="cantidad_tmp-<?php echo $id_tmp;?>" class="monto input" style="width:70px;background:#E6E6E6;text-align:center;"><?php echo round($cantidad,2);?></textarea></td>
			<td style="background:white;" class='text-center'><?php echo $nombre_producto;?></td>
            <td class='text-center' style="background:white;"><textarea  id="cod-<?php echo $id_tmp;?>" name="cod-<?php echo $id_tmp;?>" class="monto input" style="width:200px;background:#E6E6E6;text-align:center;"><?php echo $profesional;?></textarea></td> 
            <td class='text-center' style="background:white;"><textarea  id="precio_tmp-<?php echo $id_tmp;?>" name="precio_tmp-<?php echo $id_tmp;?>" class="monto input" style="width:70px;background:#E6E6E6;text-align:right;"><?php echo str_replace(",","",number_format($precio_venta_f,2));?></textarea></td>
            <td class='text-center' style="background:white;"><input class="monto total" type="text" readonly value="<?php echo str_replace(",","",number_format($precio_total_f,2));?>" style="width:100%;height:40px;text-align:right;"></td>
