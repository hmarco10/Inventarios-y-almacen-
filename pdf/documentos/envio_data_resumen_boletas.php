...
$i=0;
$sql2=mysqli_query($con, "select * from facturas,clientes where facturas.aceptado<>'Aceptada' and facturas.id_cliente=clientes.id_cliente and (facturas.resumen=0 or (facturas.resumen=1 and activo=0)) and facturas.estado_factura=2 and facturas.ven_com=1 and facturas.total_venta>0 and DATE_FORMAT(facturas.fecha_factura, '%Y-%m-%d')='$fecha' ORDER BY facturas.id_factura ASC");
        while($row2=mysqli_fetch_array($sql2)){
            ...
            $detalle[$i]["GRAVADA"]=round($row2["total_venta"]/(1+iva),2);
            $detalle[$i]["IGV"]=round(iva*($row2["total_venta"]/(1+iva)),2);
            
            if($tipo==1){
                $detalle[$i]["EXONERADO"]=round($row2["total_venta"],2);
                $detalle[$i]["GRAVADA"]=round(0,2);
                $detalle[$i]["IGV"]=round(0,2);
            }
            if($tipo==2){
                $detalle[$i]["INAFECTO"]=round($row2["total_venta"],2);
                $detalle[$i]["GRAVADA"]=round(0,2);
                $detalle[$i]["IGV"]=round(0,2);
            }
	...
	$doc5="$ruc1-$doc4";
        ...
        $messages[]= "$aceptado  se genero el Ticket Nro ".$ticket;
            ...
            $errors []= 'Hubo un error al enviar el resumen intentelo mas tarde'.$mensaje['mensaje'].$tt;
        }
        if (isset($errors)){
